...
from app.src.shemas.contact import ContactModel, ContactFavoriteModel
...
hw_path: str = str(Path(__file__).resolve().parent.parent.joinpath("app", "src"))
...
class TestContactsRepository(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_get_contacts(self):
        ...
        result = await get_contacts(skip=0, limit=10, user_id=self.user.id, favorite=favorite,
                                    db=self.session)  # type: ignore
    ...
    async def test_get_contact_search_birthday(self):
        ...
        self.assertListEqual(result, contacts[:-1])
